...
if __name__ == "__main__":
    ...
    IMAGE_ID = 1
    # path
    root = '/home/ailab/Workspace/AIHUB/pose-anno-tool/object-pose-annotator/data/220823_sample_data'
    root_object = os.path.join(root, 'models')
    root_data = os.path.join(root, 'raw', '{:06d}')
    ...
    with open(path_anno_cam.format(SCENE_ID)) as gt_file:
        anno_cam = json.load(gt_file)
    anno_cam = anno_cam[str(IMAGE_ID)] # type: dict

    # load object pose annotation
    with open(path_anno_obj.format(SCENE_ID, SCENE_ID)) as gt_file:
        anno_obj = json.load(gt_file)
    anno_obj = anno_obj[str(IMAGE_ID)] # type: list

    print(type(anno_cam), anno_cam.keys())
    print(type(anno_obj), len(anno_obj), anno_obj[0].keys())
    ...
    ###############################
    # generate offscreen renderer
    rgb_img = cv2.imread(path_rgb.format(SCENE_ID, IMAGE_ID))
